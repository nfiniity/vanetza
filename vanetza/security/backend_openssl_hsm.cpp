...
EcdsaSignature BackendOpenSslHsm::sign_data(const ecdsa256::PrivateKey& key, const ByteBuffer& data)
{
...
ByteBuffer BackendOpenSslHsm::encrypt_data(const ecdsa256::PublicKey& key, const ByteBuffer& data)
{
    auto pub_key = internal_public_key(key);

    // TODO: implement
    return data;
}

bool BackendOpenSslHsm::verify_data(const ecdsa256::PublicKey& key, const ByteBuffer& data, const EcdsaSignature& sig)
{
    auto digest = calculate_digest(data);
...
boost::optional<Uncompressed> BackendOpenSslHsm::decompress_point(const EccPoint& ecc_point)
{
...
std::array<uint8_t, 32> BackendOpenSslHsm::calculate_digest(const ByteBuffer& data) const
{
...
openssl::Key BackendOpenSslHsm::internal_private_key(const ecdsa256::PrivateKey& generic) const
{
...
openssl::Key BackendOpenSslHsm::internal_public_key(const ecdsa256::PublicKey& generic) const
{
    openssl::Key key(NID_X9_62_prime256v1);
